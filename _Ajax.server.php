...
function form_adjuntos_pedi( $codpedi, $idempresa, $idsucursal){
        ...
        if ($oIfx->Query($sql_adj)) {
            if ($oIfx->NumFilas() > 0) {
                do {
                    ...
                    $btn_adj='<span class="btn btn-danger btn-sm" title="Eliminar" value="Eliminar" onClick="javascript:elimina_adjpedi(' . $id_adj . ', '.$codpedi.', '.$idempresa.', '.$idsucursal.');">
				<i class="glyphicon glyphicon-remove"></i>
				</span>';

                    $sHtmladj .='<tr>';
                    $sHtmladj .='<td align="center">'.$a.'</td>';
                    $sHtmladj .='<td align="center">'.$imagen.'</td>';
                    $sHtmladj .='<td align="center">'.$btn_adj.'</td>';
                    $sHtmladj .='<tr>';
    ...
    //CAMPOS ADJUNTO
    $campos .='<div class="row"><div class="col-xs-12 col-sm-12 col-md-5">
                                    <div class="form-group">
                                        <label class="control-label" for="archivo_guia">Cargar Adjunto:</label>
                                        <input type="file" name="archivo_pedi" id="archivo_pedi" onchange="upload_image(id);">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-5">
                                    <div class="form-group">
                                        <span class="btn btn-success btn-sm" title="Agregar" value="Agregar"  style="margin-top:20px;"onclick="agregar_adjuntos_pedi('.$codpedi.', '.$idempresa.', '.$idsucursal.');">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title"> ADJUNTOS PEDIDO DE COMPRA Nro: '.$codpedi.' </h4>
                            '.$botones.'
                        </div>
...
function form_anular($id, $empresa, $sucursal)
{
    ...
    <div class="col-xs-12 col-sm-12 col-md-5">
               <div class="form-group">
                   ...
                   <textarea id="det_anula" name="det_anula" class="form-control" rows="5" cols="30" ></textarea>
               </div>
               </div>
    ...
    $sHtml .= '<div class="modal-dialog modal-lg" >
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4>ANULACION SOLICITUD DE COMPRA: ' . $id . '</h4>
            </div>
            <div class="modal-body">';
    ...
    $sHtml .= '</div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" onclick="anula_solicitud(\'' . $id . '\', ' . $empresa . ', ' . $sucursal . ')">Anular</button>
                            <button type="button" class="btn btn-danger" data-dismiss="modal"  >Cerrar</button>
                        </div>
...
function reporte_solicitudes($aForm = '')
{
    ...
    try {
        ...
        $table_op .= '                      id="btnGenerarReporteSolicitudes" title="Generar reporte">';
        $table_op .= '                <i class="fa fa-play"></i> Generar reporte';
        $table_op .= '              </button>';
        $table_op .= '            </div>';
...
function adjuntos_solicitud($id, $empresa, $sucursal, $tipo)
{
    ...
    ////CARGA DE ADJUNTOS////
    $k = 1;
    $sqlpedi = "select dped_prod_nom, dped_det_dped, dped_cod_prod, dped_adj_dped  from saedped where dped_cod_pedi='$id' and dped_cod_empr= $empresa and dped_cod_sucu=$sucursal";
    ...
    if ($oIfx->Query($sqlpedi)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $sHtml .= '<td align="center">' . $prod_cod . '</td>';
                $sHtml .= '<td align="center">' . $prod . '</td>';
    ...
    if ($oIfx->Query($sqlpedi)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $sHtml .= '<td align="center">' . $cantapro . '</td>';
...
function genera_formulario_pedido($sAccion = 'nuevo', $aForm = '', $cod_sol = 0, $empr_sol = 0, $sucu_sol = 0, $id_req = 0)
{
    ...
    switch ($sAccion) {
        case 'nuevo':
            ...
            $ifu->AgregarCampoTexto('solicitado', 'Solicitado por|left', true, $empl_nom, 380, 280, true);
                        ...
                        <div class="producto-form-panel producto-layout" style="background: #f8f9fb; border: 1px solid #e0e0e0; border-radius: 6px; padding: 12px 15px;">
                            ...
                            <div class="row fila-producto" id="filaProductoRegistrado">
                                ...
                                <div class="col-xs-12 col-sm-4 col-md-2" id="slotCantidadRegistrado">
                                    <div id="contenedorCantidadUnidad">
...
function mostrar_grid($idempresa)
{
    ...
    foreach ($aDataGrid as $aValues) {
        ...
        $aDatos[$cont]['Detalle'] = nl2br(htmlspecialchars($detalleConSaltos, ENT_QUOTES, 'UTF-8'));
